...
int main ( int argc, char** argv ) {
  ...
  while( ++phase < 5 ){
    ...
    while( 1 ){
      ...
      // cout << q.size() << endl;
      count++;
      // cout << "count: " << count << " and ";//<< oldState << endl;
      // for(int i=0; i<oldId.size(); i++)
      //   cout << oldId[i] << " ";
      // cout << oldDir << " " << q.size() << endl;
      // if(count > 120)
      //   return 0;
      //--- Apply all applicable moves to it and handle the new state.
      for( int move=0; move<18; move++ ){
        // cout << (applicableMoves[phase] & (1 << move)) << endl;
