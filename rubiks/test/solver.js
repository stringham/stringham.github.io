	...
	for(var i=0; i<20; i++){
		...
		while((this.currentState[i] = goal.indexOf(cubie)) == -1){
			...
			if(this.currentState[i+20] > 2){
				this.currentState = "Invalid state. Invalid or duplicate cubie: " + cubie;
				return false;
			}
		}
...
RubiksCubeSolver.prototype.verifyState = function() {
	if(!Array.isArray(this.currentState.length))
		return false;
	//orientation of edges
	var sum = 0;
	goog.array.forEach(this.currentState.slice(20,32), function(edge){
		sum+=edge;
	});
	if(sum % 2 != 0){
		//edge orientation
		this.currentState = "Edge Orientation Mismatch";
		return false;
	}
	sum = 0;
	//orientation of corners
	goog.array.forEach(this.currentState.slice(32,40), function(edge){
		sum+=edge;
	});
	if(sum % 3 != 0){
		//corner orientation
		this.currentState = "Corner Orientation Mismatch";
		return false;
	}
